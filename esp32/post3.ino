...
void loop() {
    ...
    char *body = (char *) malloc(sizeof(char) * ARRAY_SIZE);
...
void appendToBuffer(char* buffer, size_t  offset, String content) {
    for (size_t idx = 0; idx < content.length(); idx++) {
        buffer[offset + idx] = content[idx];
    }
    buffer[offset + content.length()] = '\0';
}


void appendToBuffer(char* buffer, size_t  offset, uint8_t *array, size_t length) {
    for (size_t idx = 0; idx < length; idx++) {
        buffer[offset + idx] = (char) array[idx];
    }
    buffer[offset + length] = '\0';
}

void generateRequestBody(char *buffer) {
    ...
    bufferOffset = generateRequestHeader(buffer);

    // append image to request
    appendToBuffer(buffer, bufferOffset, fb->buf, fb->len);
    bufferOffset +=  fb->len;

    generateRequestTail(buffer, bufferOffset);
}
...
*/
size_t generateRequestHeader(char *buffer) {
    ...
    String completeHeader = cameraNameHeader + requestHead;
    appendToBuffer(buffer, 0, completeHeader);
    return completeHeader.length();
}

/**
* Returns the tail of form. (e.g. closing tag of the form)
*/
size_t generateRequestTail(char *buffer, size_t offset) {
    // USE_SERIAL.print("getRequestTail()");

    String tail = "\r\n--" + formBoundary + "--\r\n\r\n";
    appendToBuffer(buffer, offset, tail);
    size_t val = offset + tail.length();
    return val;
}
