...
namespace MoodCap
{
    public class PostImage : IHttpFunction
    {
        private readonly ILogger _logger;
        private StorageClient _storage;

        public PostImage(ILogger<PostImage> logger) {
            _logger = logger;
        ...
        public async Task HandleAsync(HttpContext context)
        {
            ...
            if(request.Form.Files.GetFile("Content") == null) {
                response.StatusCode = (int) HttpStatusCode.BadRequest;
                await response.WriteAsync("The request does not contain an file named Content");
                return;
            }

            IFormFile cameraImage = request.Form.Files.GetFile("Content");
